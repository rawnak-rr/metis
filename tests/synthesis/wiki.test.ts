...
describe("wiki validation and lint", () => {
  ...
  it("preserves synthesis that fails only lexical validation during repair", async () => {
    ...
    const sourceId = ingested.source.id;
    const page = await metis.wiki.upsertWikiPage({
      title: "Mitochondria",
      summary: "Cellular energy production.",
    ...
    const result = await metis.knowledgeRepair.repairKnowledge();
  ...
  it("recovery output with quoted markdown passes structural validation", async () => {
    ...
    const first = await metis.knowledgeRepair.repairKnowledge();
    ...
    expect(lint.issues.filter((issue) =>
      ...
      .toEqual([]);
    const second = await metis.knowledgeRepair.repairKnowledge();

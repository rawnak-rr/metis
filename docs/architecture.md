...
| `contracts/` | the vault data model: zod schemas, versioned migrations, source-type descriptors, the citation token grammar |
| `vault/` | the filesystem boundary: state, config, the write queue and lock, managed file writes, generated templates, backups, read-only views |
...
| `repair/` | the upgrade and recovery boundary, and the generated Agent Skills bundle |
...
Two edges are worth naming. Ingestion declares the `SourceIndexWriter` port that
search implements, so warming the index after an ingest is an effect ingestion
requests rather than a dependency it owns. Repair sits above everything because
it rebuilds what the other layers derive, and it reads evidence through the same
verified reader, so a missing or modified raw source stops a repair instead of
being papered over from derived data.
